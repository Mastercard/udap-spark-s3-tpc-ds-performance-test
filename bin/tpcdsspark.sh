...
check_createtables() {
  ...
  bin/spark-sql --master yarn --deploy-mode client ${DRIVER_OPTIONS} ${EXECUTOR_OPTIONS} --conf spark.sql.catalogImplementation=hive -f ${TPCDS_WORK_DIR}/row_counts.sql > ${TPCDS_WORK_DIR}/rowcounts.out 2>&1
...
function create_spark_tables {
  ...
  if [ "$result" -ne 1 ]; then 
    ...
    ProgressBar 2 122
    bin/spark-sql --master yarn --deploy-mode client ${DRIVER_OPTIONS} ${EXECUTOR_OPTIONS} --conf spark.sql.catalogImplementation=hive -f ${TPCDS_WORK_DIR}/create_database.sql > ${TPCDS_WORK_DIR}/create_database.out 2>&1
    script_pid=$!
    bin/spark-sql --master yarn --deploy-mode client ${DRIVER_OPTIONS} ${EXECUTOR_OPTIONS} --conf spark.sql.catalogImplementation=hive -f ${TPCDS_WORK_DIR}/create_tables_work.sql > ${TPCDS_WORK_DIR}/create_tables.out 2>&1 &
    script_pid=$!
    cont=1
    error_code=0
    while [  $cont -gt 0 ]; do
