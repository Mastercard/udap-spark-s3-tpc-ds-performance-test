...
EXECUTOR_OPTIONS="--executor-memory 2g --num-executors 3 --conf spark.executor.extraJavaOptions=-Dlog4j.configuration=file:///${OUTPUT_DIR}/log4j.properties --conf spark.sql.crossJoin.enabled=true"
...
do
  num=`printf "%02d\n" $i`
  bin/spark-sql --master yarn --deploy-mode client ${DRIVER_OPTIONS} ${EXECUTOR_OPTIONS} ${ADDITION_SPARK_OPTIONS} -database TPCDS -f ${OUTPUT_DIR}/query${num}.sql > ${OUTPUT_DIR}/query${num}.res 2>&1 
